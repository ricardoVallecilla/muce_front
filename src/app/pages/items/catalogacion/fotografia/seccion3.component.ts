...
@Component({
  selector: 'foto-seccion3',
  templateUrl: './seccion3.html'
})
export class Seccion3FotoComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: { [propKey: string]: SimpleChange }) {
    ...
    if(changes.submitted!=undefined){
      ...
      if(this.submitted>0){
        ...
        if (valido==1){
          this.validacionFormulario.emit({indentificador:1,tab:"CONTEXTO TÉCNICO DE LA FOTOGRAFÍA",valido:true})
        }else{
          this.validacionFormulario.emit({indentificador:1,tab:"CONTEXTO TÉCNICO DE LA FOTOGRAFÍA",valido:false})
        }
      }
