...
@Component({
  selector: 'zoo-seccion2',
  templateUrl: './seccion2.html'
})
export class Seccion2ZooComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.detalle!=null ){
        ...
        this.descargarFoto(16);
        this.descargarFoto(17);
    }
  }
  ...
  ngOnChanges(changes: { [propKey: string]: SimpleChange }) {
    ...
    if(changes.submitted!=undefined){
      ...
      if(this.submitted>0){
        ...
        if (valido==1){
          this.validacionFormulario.emit({indentificador:9,tab:"DATOS DE RECOLECCIÓN",valido:true})
        }else{
          this.validacionFormulario.emit({indentificador:9,tab:"DATOS DE RECOLECCIÓN",valido:false})
        }
      }
