...
@Component({
  selector: 'arte-seccion2',
  templateUrl: './seccion2.html'
})
export class Seccion2ArteComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: { [propKey: string]: SimpleChange }) {
    ...
    if(changes.submitted!=undefined){
      ...
      if(this.submitted>0){
        ...
        if (valido==1){
          this.validacionFormulario.emit({indentificador:1,tab:"JUICIO VALORATIVO",valido:true})
        }else{
          this.validacionFormulario.emit({indentificador:1,tab:"JUICIO VALORATIVO",valido:false})
        }
      }
